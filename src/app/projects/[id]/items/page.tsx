...
import { ArrowLeft, ChevronDown, CheckCircle, AlertCircle, Clock } from 'lucide-react'
          ...
          {Object.entries(filterCounts).map(([key, count]) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {key === 'matched' && <span className="text-base">✅</span>}
                {key === 'qs_review' && <span className="text-base">👁️</span>}
                {key === 'pending' && <span className="text-base">⏳</span>}
                {key === 'failed' && <span className="text-base">❌</span>}
                <span>
                  {key === 'all' ? 'All Items' : 
                   key === 'matched' ? 'Matched' :
                   key === 'qs_review' ? 'QS Review' :
                   key === 'pending' ? 'Pending' :
